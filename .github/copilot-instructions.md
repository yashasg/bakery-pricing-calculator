...
Sources use `__APP_NAME__` (Swift target name, scheme, directory) and `__BUNDLE_ID__` (reverse-DNS bundle ID) until bootstrap runs:
...
├── app/
│   ├── app.xcodeproj/
│   ├── __APP_NAME__/             # Main app target sources
│   │   ├── __APP_NAME__App.swift # @main entry point; sets .satoshiBody as default font
│   │   ├── Components/           # Reusable UI (Font+Satoshi.swift, etc.)
│   │   ├── Views/                # Feature views
│   │   └── Assets.xcassets/
│   ├── __APP_NAME__Tests/        # Unit tests (XCTest)
│   ├── __APP_NAME__UITests/      # UI tests (XCUIApplication)
...
**File placement:**
- New Swift files → `app/__APP_NAME__/` (or `Views/` / `Components/`)
- Unit tests → `app/__APP_NAME__Tests/`
- UI tests → `app/__APP_NAME__UITests/`
- Docs → `docs/`
...
- `__APP_NAME__App.swift` is `@main`, wraps a single `WindowGroup`, sets `.satoshiBody` as default font for all descendants.
...
- Unit tests in `app/__APP_NAME__Tests/` (XCTestCase). Name: `test<Scenario>()`.
- UI tests in `app/__APP_NAME__UITests/` (XCUIApplication). Match elements via `accessibilityIdentifier`.
...
- **Tokens** — never hardcode app names or bundle IDs; use `__APP_NAME__` / `__BUNDLE_ID__` in template files.
...
- **File placement** — new Swift files under `app/__APP_NAME__/`; tests under the right test target; docs under `docs/`.
